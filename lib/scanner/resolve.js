...
(function() {
  var closest_mod_folder, config, debug, dirs, error, exts, fs, i, info, len, log, logger, mod_kinds, mod_manifests, path, plugin, plugins, resolve_file, resolve_id, resolve_index, resolve_module, upath, warn;
  ...
  module.exports = function(filepath, id) {
    var caller, file, index, j, kind, len1, manifest;
    id = id.replace(/\.js$/m, '');
    for (index = j = 0, len1 = mod_kinds.length; j < len1; index = ++j) {
  ...
  resolve_module = function(kind, manifest, filepath, id) {
    var file, idpath, json, location, main, map, mod, nmods, non_recurse, ref;
    if (id == null) {
      id = '';
    }
    if (id === '') {
    ...
    if (filepath !== path.parse(filepath).root && non_recurse !== true) {
      return resolve_module(kind, manifest, upath.join(filepath, '..'), id);
    }
  };
